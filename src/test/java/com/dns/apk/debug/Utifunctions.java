...
public class Utifunctions extends BaseClass {
	 ...
	 public static void scrollDown(){
	    ...
	    public static void scrollNClick(By listItems, String Text){
	        ...
	        while(true){
	            for(WebElement el: AppDriver.getDriver().findElements(listItems)){
	                if(el.getAttribute("text").equals("Personal Information")){
	                    el.click();
	                    flag=true;
	                    break;
	                }
	    ...
	    public static void scrollIntoView(String Text){
	        ((AndroidDriver<MobileElement>) AppDriver.getDriver()).findElementByAndroidUIAutomator("new UiScrollable(new UiSelector().scrollable(true).instance(0)).scrollIntoView(new UiSelector().text(\"" + Text + "\").instance(0))").click();
	    }
